...
url+="&xdomain_urls="+encodeURIComponent(JSON.stringify(urls))}if(options.azureWorkaround)url+="&azureWorkaround=1";if(!options.useDownloader)url+="&useDownloader=0";if(url.length>0&&url[0]==="&")url="#"+url.slice(1);return url},_createHTML5:function(){var me=this;var iframeSource=this.options.html5Path+this._getHTML5OptionsURL();var $rcFrame=$(document.createElement("iframe"));$rcFrame.attr({id:this.rcId,src:iframeSource,frameborder:0,width:"100%",height:"100%",allowFullScreen:true,webkitallowfullscreen:true,
mozallowfullscreen:true});var outerWindow=window;$rcFrame.load(function(){me.instance=this.contentWindow.readerControl;outerWindow.CoreControls=outerWindow.CoreControls||{};outerWindow.CoreControls.DisplayModes=this.contentWindow.CoreControls.DisplayModes;var iframe=this;if(typeof me.instance==="undefined")this.contentWindow.$(this.contentDocument).bind("viewerLoaded",function(event){me.instance=iframe.contentWindow.readerControl;me._viewerLoaded(iframe)});else me._viewerLoaded(iframe)});$(this.element).append($rcFrame);
return $rcFrame},_createHTML5Mobile:function(){var me=this;var iframeSource=this.options.html5MobilePath+this._getHTML5OptionsURL();var $rcFrame=$(document.createElement("iframe"));$rcFrame.attr({id:this.rcId,src:iframeSource,frameborder:0});$rcFrame.css("width","100%").css("height","100%");$rcFrame.load(function(){me.instance=this.contentWindow.readerControl;var iframe=this;if(typeof me.instance==="undefined")this.contentWindow.$(this.contentDocument).bind("viewerLoaded",function(event){me.instance=
iframe.contentWindow.readerControl;me._viewerLoaded(iframe)});else me._viewerLoaded(iframe)});$(this.element).append($rcFrame);return $rcFrame},_init:function(){},getInstance:function(){return this.instance},loadErrorPage:function(){if(typeof this.options.errorPage==="undefined")$(this.element).append("<h2 style='text-align:center;margin-top:100px'>We're sorry, an error has occurred.</h2>");else $(this.element).load(this.options.errorPage)},getSideWindowVisibility:function(){return this.getInstance().getShowSideWindow()},
...
else if(layoutMode===displayModes.Continuous)return PDFTron.WebViewer.LayoutMode.Continuous;else if(layoutMode===displayModes.Facing)return PDFTron.WebViewer.LayoutMode.Facing;else if(layoutMode===displayModes.FacingContinuous)return PDFTron.WebViewer.LayoutMode.FacingContinuous;else if(layoutMode===displayModes.Cover)return PDFTron.WebViewer.LayoutMode.FacingCoverContinuous;else if(layoutMode===displayModes.CoverFacing)return PDFTron.WebViewer.LayoutMode.FacingCover;else return undefined},setLayoutMode:function(layoutMode){var displayModes=
...
option:function(key,value){if($.isPlainObject(key))this.options=$.extend(true,this.options,key);else if(key&&typeof value==="undefined")return this.options[key];else this.options[key]=value;return this},_correctRelativePath:function(path){var curdir=window.location.pathname.substr(0,window.location.pathname.lastIndexOf("/"));var pattern=/^(https?:\/\/|file:\/\/|\/|blob:|%2F)/i;return pattern.test(path)?path:curdir+"/"+path},_testHTML5:function(){try{var c=document.createElement("canvas");return c&&
c.getContext("2d")}catch(e){return false}},_supports:function(type){if(type===this.selectedType)return true;for(var i=1;i<arguments.length;i++)if(arguments[i]===this.selectedType)return true;return false},_testCORS:function(){return"XMLHttpRequest"in window&&"withCredentials"in new XMLHttpRequest},isMobileDevice:function(){return this.scrollbarWidth()===0&&navigator.userAgent.match(/Edge/i)||(navigator.userAgent.match(/Android/i)||(navigator.userAgent.match(/webOS/i)||(navigator.userAgent.match(/iPhone/i)||
...
return result},isChrome:function(){var opera=window.navigator.userAgent.match(/OPR/);var maxthon=window.navigator.userAgent.match(/Maxthon/);var edge=window.navigator.userAgent.match(/Edge/);return window.navigator.userAgent.match(/Chrome\/(.*?) /)&&(!opera&&(!maxthon&&!edge))},isSameOrigin:function(url){var loc=window.location;var a=document.createElement("a");a.href=url;if(a.host==="")a.href=a.href;var locPort=window.location.port;var aPort=a.port;if(a.protocol==="http:"){aPort=aPort||"80";locPort=
locPort||"80"}else if(a.protocol==="https:"){aPort=aPort||"443";locPort=locPort||"443"}return a.hostname===loc.hostname&&(a.protocol===loc.protocol&&aPort===locPort)}};if(typeof $.widget==="function")$.widget("PDFTron.webViewer",PDFTron.WebViewer.prototype);PDFTron.WebViewer.LayoutMode={Single:"SinglePage",Continuous:"Continuous",Facing:"Facing",FacingContinuous:"FacingContinuous",FacingCover:"FacingCover",FacingCoverContinuous:"CoverContinuous"};
