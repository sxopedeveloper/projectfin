...
class Account extends Admin_main 
{
    ...
    public function new_account () // PAGE VIEW
	{
		$data = $this->data;
		$data['title'] = 'Accounts - Create New';
		$this->load->helper(array('form'));
		$this->load->view('admin/dealer_add', $data);
	}
	...
	public function verify_account_creation ()
	{
		...
		if($this->form_validation->run() == FALSE) {
            ...
            $validation_errors = str_replace("</p>", "", $validation_errors);
            $Return['validation_msg'] = $validation_errors;
			...
			$ins_data = array(
                'type'                  =>  $this->db->escape_str($user_type),
                'admin_type'            =>  $this->db->escape_str($admin_type),
                'status'                =>  1,
                'username'              =>  $this->db->escape_str($username),
                'password'              =>  $this->db->escape_str($password),
                'name'                  =>  $this->db->escape_str($name),
                'abn'                   =>  $this->db->escape_str($abn),
                ...
                'bank_acct_name'        =>  $this->db->escape_str($bank_acct_name),
                'bank_acct_bsb'         =>  $this->db->escape_str($bank_acct_bsb),
                'bank_acct_num'         =>  $this->db->escape_str($bank_acct_num),
                'account_holders_name'  =>  $this->db->escape_str($acct_holders_name),
                'dealer_license'        =>  $this->db->escape_str($dealer_license),
                'created_at'            =>  date("Y-m-d H:i:s")
            
            );
           $user_id = $this->user_model->insert('users',$ins_data);
			...
			else if ($user_type==5) { $ref_url = "fquser"; }
			//header("Location: " . site_url('account/'.$ref_url));
            $Return['success'] = "Added";
			$Return['ref_url'] = site_url('account/'.$ref_url);
    ...
    public function create_dealer_profile() {
        $input_arr = $this->input->post();
        //echo '<pre>';print_r($input_arr);exit;
        
        $this->load->library('form_validation');
		$this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email|xss_clean|callback_check_username');
		$this->form_validation->set_rules('dealership_name', 'Dealership Name', 'trim|required');
        if($input_arr['user_typr'] == '1'){
		  $this->form_validation->set_rules('dealership_brand', 'Dealership Brand', 'required');
        }
                ...
                foreach ($contact_data_keys as $contact_key) {
                    if(!empty($input_arr['contact_fullname'][$i]) && !empty($input_arr['contact_email'][$i])) {
                        ...
                        if($contact_key=="contact_id") {
                            ...
                            if(isset($input_arr['contact_is_primary'][$contact_id])){
                                $one_data['contact_is_primary'] = $input_arr['contact_is_primary'][$contact_id];
                            }else{
                                $one_data['contact_is_primary'] = 0;
                            }
        ...
        //echo '<pre>';print_r($input_arr);exit;
        $update_res = $this->user_model->create_dealer_profile($input_arr);
        if($update_res) {
            $return['success'] = "Added";
        } else {
    ...
    public function check_username ()
	{
        ...
        $result = $this->user_model->check_username_availability($username, $this->input->post('user_type'));
	...
	public function fquser ($start = 0) // LIST VIEW
	{
		
		$data = $this->data;
		$data['title'] = 'FQ user account';

		$limit = 20;
		$count_result = $this->user_model->get_fquser_count($_GET);
		$data['fquser'] = $this->user_model->get_fquser($_GET, $start, $limit);

		$this->load->library('pagination');
		$config['base_url'] = site_url('account/fquser/');
		$config['total_rows'] = $count_result['cnt'];
		$config['per_page'] = $limit;
		$this->pagination->initialize($config); 
		$data['links'] = $this->pagination->create_links();
		...
		$this->load->view('admin/fquser', $data);
	}
										...
										<tbody>';
											...
											foreach($data['dealers'] as $dealer)
											{
												...
												$floatdealer .='<tr id="dealer_row_'.$dealer->id_user.'">
													<td class="text-center">';
														...
														$floatdealer .='<a href="javascript:void(0)" id="activate_button_'.$dealer->id_user.'" onclick="activate_dealer('.$dealer->id_user.')" data-toggle="tooltip" data-placement="top" title="Activate Dealer" '.$activate_button_hidden.'>
															<i class="fa fa-check"></i>
														</a>
														<a href="javascript:void(0)" id="deactivate_button_'.$dealer->id_user.'" onclick="deactivate_dealer('.$dealer->id_user.')" data-toggle="tooltip" data-placement="top" title="Deactivate Dealer"'.$deactivate_button_hidden.'>
													...
													<td class="text-center">';
														...
														$floatdealer .='<a href="javascript:void(0)" id="activate_gold_'.$dealer->id_user.'" onclick="gold_control('.$dealer->id_user.', 1)" data-toggle="tooltip" data-placement="top" title="Mark as Gold" '.$gold_btn_hidden.'>
															<i class="fa fa-star" ></i>
														</a>
														<a href="javascript:void(0)" id="deactivate_gold_'.$dealer->id_user.'" onclick="gold_control('.$dealer->id_user.', 0)" data-toggle="tooltip" data-placement="top" title="Unmark as Gold" '.$ungold_btn_hidden.'>
													...
													<td class="text-center">';
														...
														$floatdealer .='<a href="javascript:void(0)" id="show_deposit_'.$dealer->id_user.'" onclick="deposit_visibility_control('.$dealer->id_user.', 1)" data-toggle="tooltip" data-placement="top" title="Show Deposit" '.$show_deposit_btn_hidden.'>
															<i class="fa fa-dollar" ></i>
														</a>
														<a href="javascript:void(0)" id="hide_deposit_'.$dealer->id_user.'" onclick="deposit_visibility_control('.$dealer->id_user.', 0)" data-toggle="tooltip" data-placement="top" title="Hide Deposit" '.$hide_deposit_btn_hidden.'>
															<i class="fa fa-dollar" style="color: #FFD700;" ></i>
														</a>
													</td>
													<td class="text-center">
													<a href="javascript:void(0)" class="open-editdealer" data-dealer_id="'.$dealer->id_user.'">
															<i class="fa fa-pencil-square-o" data-toggle="tooltip" data-placement="top" title="Edit Dealer"></i>
														</a>
													</td>
													<td class="text-center">
														<a href="javascript:void(0)" data-id="'.$dealer->id_user.'" class="delete_dealer" onclick="delete_dealer('.$dealer->id_user.')">
											...
											<a href="mailto:'.$dealer->username.'" target="_top">'.$dealer->username.'</a><br />
												<i class="fa fa-car"></i> <span id="dealer_dealership_brand_'. $dealer->id_user.'">'.$dealer->dealership_brand.'</span>
													...
													<td>
														<i class="fa fa-user"></i> <span id="dealer_name_'.$dealer->id_user.'">'.$dealer_name.'</span><br />
														<i class="fa fa-mobile"></i> &nbsp;<span id="dealer_mobile_'. $dealer->id_user.'">'.$dealer_mobile.'</span><br />
														<i class="fa fa-phone"></i> <span id="dealer_phone_'.$dealer->id_user.'">'.$dealer_phone.'</span>
													</td>													
													<td>
    ...
    public function dealer ($dealer_id) // MODAL VIEW
    {
		...
		$primary_contact_checked = $dealer['is_primary_contact'] == 1 ? 'checked' : '';
		if($dealership_contact_boxs != '')
		{
			...
			if(is_array($dealership_contact_boxs_arr))
			{
				...
				$dealership_contact_boxs_html .= "<div class='dealer-input-check'>
                                                    <span class='primary-contact' style='padding-right: 15px;'>Is Primary Contact</span>
                                                    <input type='checkbox' {$primary_contact_checked} class='check-new-one in-check is_primary_contact' id='is_primary_contact' value='1' name='is_primary_contact' required=''>
			...
			foreach ($dealer_contacts_arr as $row) {
				...
				if(!empty($row))
				{
					...
					if(isset($row['contact_boxs']))
					{
                        ...
                        $contact_is_primary = isset($contact_is_primary) && $contact_is_primary == 1 ? 'checked' : '';
                        
                        //$contact_is_primary = 'checked';
						$dealer_contacts_html .= "<div class='dealer-input-check 11'>
                                                    <span class='primary-contact' style='padding-right: 15px;'>Is Primary Contact</span>
                                                    <input type='checkbox' {$contact_is_primary} class='check-new-one in-check is_primary_contact' id='is_primary_contact{$n}' value='1' name='contact_is_primary[{$n}]'>
                                                    <label for='is_primary_contact{$n}' data-toggle='tooltip' data-placement='top' title='Is Primary Contact' data-original-title='Is Primary Contact'>
                                                        <span class='border-span'><i class='fa fa-check' aria-hidden='true'></i></span>
                                                    </label>
                                                </div>";
						$dealer_contacts_html .= "</div>";
        ...
        $states_full = array('ACT' => 'ACT - Australian Capital Territory','NSW' => 'NSW - New South Wales',
				'NT' => 'NT - Northern Territory', 'QLD' => 'QLD - Queensland',
				'SA' => 'SA - South Australia', 'TAS' => 'TAS - Tasmania',
				'VIC' => 'VIC - Victoria', 'WA' => 'WA - Western Australia',
			);
                        ...
                        <div class="form-group">
							<div class="dealer-detail-form-group">
                                ...
                                <select class="form-control input-md" name="state" type="text">
                                    ...
                                    foreach ($states_full as $state_key => $state_val) {
                                        $dealer_details .= '<option '.((trim($state_key) == $dealer['state']) ? ' selected="selected" ' : "").' value="'.$state_key.'">'.$state_val .'</option>';	
                                    }
                                $dealer_details .= '</select>
							</div>														
						</div>
						<div class="form-group">
							<div class="dealer-detail-form-group">
	...
	public function update_dealer_profile () // MODAL ACTION
	{
		...
		$contact_data = array();
		$contact_mail_id = '';
		$contact_data_keys = array("contact_id","contact_title","contact_fullname","contact_email","contact_mobile","contact_landline","contact_is_primary");
			...
			for($i=0;$i<count($input_arr['contact_title']);$i++)
			{
				...
				foreach ($contact_data_keys as $contact_key) {
					if(!empty($input_arr['contact_fullname'][$i]) && !empty($input_arr['contact_email'][$i]))
					{
                        ...
                        $contact_id = $input_arr['contact_id'][$i];
                        if($contact_key=="contact_id")
						{
							$contact_id = $input_arr[$contact_key][$i];
							$contact_boxs = '';
    ...
    public function get_del(){
        $dealer = $this->user_model->get_dealer(589);
        ...
        echo '<pre>';print_r($dealer_contacts_arr);
        echo '<br>';
        echo '<br>';
        echo '<br>';
        ...
        echo '<pre>';print_r($dealership_contact_boxs_arr);exit;
