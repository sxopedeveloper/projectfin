...
class Fapplication extends Admin_main 
{
	...
	{	
		...
		$return = $this->fapplication_model->delete_cal_item($id);
		echo  $return;
	}
	...
	public function allocation_status_change()
	{
		...
		/*print_r($fapplication_ids );echo '<br>';echo  $status;exit;*/
...
//echo json_encode($input_arr); die();
		...
		foreach ($dealer_id_arr as $dealer_id)
		{
			if ($dealer_id <> 0)
			{
				...
				if (isset($user_detail['username']))
				{
					...
					$this->send_quote_request_mail($dealer_id,$input_arr['id_lead'],$rand_key);
	...
	public function send_quote_request_mail($dealer_id,$lead_id,$rand_key){
		$data = $this->data;
		$user_details = $this->user_model->get_dealer_for_email($dealer_id);
		$lead_details = $this->lead_model->get_lead_for_email($lead_id);
		$link_url = site_url("dealer/add_document_tender/".$rand_key);
		$from_email = $data['company']['company_email'];
		$from_name = $data['company']['company_name'];
		$to = "devakshay89@gmail.com";//$dealer['username'];
		$subject = $data['company']['company_name']." - Submit Tender";
		$content = '
			<p style="line-height: 1.5">
				Please Click Below Button to Submit Tender Document. <b></b>  
			</p>
			 <p>
	...
	public function start_tender_new () // MODAL ACTION // AUDIT TRAIL //
	{
		...
		if ($insert_quote_request_result)
		{
			...
			$response = $this->send_tender_confirmation_email($input_arr['id_lead']);//temporary commented tanvi
	...
	public function send_tender_confirmation_email ($lead_id)
	{
		...
		<p style="line-height: 1.5">
			As discussed multiple dealers will be given the opportunity to quote on your new vehicle detailed below. Please take a moment to review the details, if there is an error or an amendment required please reply to this email. To proceed click Confirm & Start Tender below to initiate the request for quotes.
		</p>
		<p style="line-height: 1.5">
		...
		<p style="line-height: 1.5">
			<a href="'.site_url("Process/confirmtender/{$lead_id}").'" style="background-color: #58c603;text-decoration: none;color: #fff;padding: 10px 25px;border-radius: 8px;" >Confirm & Start Tender</a>
		</p>
		<p style="line-height: 1.5">
			Kind Regards, 
		</p>';
		...
		{	//echo $lead['qs_email'];
			...
			$return = $this->mm_send_templated_email($lead['qs_email'], $lead['qs_name'], $email_recipients, $data['company']['company_name'].':
Tender Confirmation Email ('.$cq_number.')', $content);
			...
			if($return ==1){return "success";}else{echo "Error";}		
		}
	}	
...
$traidin .='</div>';
	...
	//Qm Functionality end
		...
		$fapplication_summary_new = '<div class="col-md-3">
									<div class="clearfix">
										<input type="text" name="new_lead_name" value="'.$fq_dealer_data['name'].'" placeholder="Enter name" class="h3-text form-control num_inp address_inp">
										<input type="text" name="new_lead_number" value="'.$fq_dealer_data['number'].'" placeholder="Enter contact number" class="pull-right form-control num_inp address_inp">
									</div>
									
									<div class="mt20 text-capitalize personal-detail car_detail">
										<p>Client Registered Address</p>
										<div class="form-group">
											<input type="text" name="new_lead_address" value="'.$fq_dealer_data['address'].'" placeholder="Enter Client Registered Address" class="form-control address_inp">
										</div>
										<p>postcode</p>
										<div class="form-group">
											<input type="text" name="new_lead_postcode" value="'.$fq_dealer_data['postcode'].'" placeholder="Enter Postcode" class="form-control address_inp">
										</div>										
										<p>DL No.</p>
										<div class="form-group">
								...
								<div class="col-md-3" >
									...
									<div class="mt50 text-capitalize personal-detail car_make" >
										<p>Make :</p>  <span style="color:#fff">'.$fq_dealer_data['make'].'</span>
				...
				$fapplication_summary_new .='</select>
										...
										<p>Year :</p> <span style="color:#fff">'.$fq_dealer_data['year'].'</span>
										<div class="form-group" style="display:none" >
						...
						$fapplication_summary_new .= '</select>
										...
										<p>Variant :</p> <span style="color:#fff">'.$fq_dealer_data['veriant'].'</span>
										<div class="form-group" style="display:none" >
						...
						$fapplication_summary_new .= '</select>
								...
								<div class="col-md-6">
								<p style="color: #58c603;">Landing Page URL : <span style="color:#fff">'.$fq_dealer_data['landing_page_url'].'</span ></p>
								<div class="col-md-6">
		...
		$tender ='<div class="panel-heading" style="border-bottom: 1px solid #ddd;"> <!-- Status -->
						...
						<b>Submitted:</b>'.$lead['created_at'].'
									...
									else if ($lead['status']==5)
									{
										...
										<span class="btn btn-default btn-sm ajax_button deliver_button" data-id_lead="'. $lead['id_fq_account'].'">
											Delivered
										</span>';
								...
								$tender .='<div class="row"> 	
									<div class="col-sm-12">
										<div class="alert alert-default">
											...
											<br />
											<br />
											<span class="btn btn-default btn-sm ajax_button send_tender_confirmation_button" data-id_lead="'.$lead['id_fq_account'].'" data-id_quote_request="'.$lead['id_quote_request'].'">
												Send Tender Confirmation
											</span>
											<span class="btn btn-default btn-sm ajax_button add_dealers_modal_button" data-id_lead="'.$lead['id_fq_account'].'" data-id_quote_request="'.$lead['id_quote_request'].'">
												Add Dealers
											</span>
						...
						<div class="table-responsive">
							<table class="table table-bordered table-striped table-condensed mb-none">
								<tbody>
									...
									$tender .='<tr>
												<td><a href="'.site_url('user/record/'.$tender_dealer['user_id']).'" target="_blank">'.$tender_dealer['name'].'</a></td>
						...
						<table class="table table-bordered table-striped table-condensed mb-none">
							<tbody>
								...
								if(count($tender_quotes)==0)
								{
									...
									$tender .='<tr id="norecord"><td colspan="10"><center>No records found!</center></td></tr>';
										...
										$tender .='<tr>
											...
											else
											{
												...
												$tender .='<td align="center">
													...
													<span class="ajax_button_primary update_quote_seen_status_button" data-id_lead="'.$lead['id_fq_account'].'" data-id_quote_request="'. $tender_quote['id_quote_request'].'" data-id_quote="'.$tender_quote['id_quote'].'" data-seen_status="1">
											...
											else
											{
												...
												$tender .='<td align="center">
													...
													<span class="ajax_button_primary select_winning_quote_button" data-id_lead="'.$lead['id_fq_account'].'" data-id_quote_request="'.$tender_quote['id_quote_request'].'" data-id_quote="'.$tender_quote['id_quote'].'">
...
$payment ='<section class="active" id="invoices_container"> 
	...
	public function get_email_trail()
	{
		...
		$html .= '<table class="table table-striped table-condensed mb-none" style="white-space: nowrap;" id="datatables_email_trail">
          <thead>
            <tr>
